...
from src.tong_until import load_data
from src.utils import accuracy
from src.model_homograph import NNet
...
parser.add_argument('--K', type=int, default=3, help='Propagation step')
parser.add_argument('--sample', type=int, default=2, help='Sampling times of dropnode')
parser.add_argument('--tem', type=float, default=0.8, help='Sharpening temperature')
parser.add_argument('--lam', type=float, default=1.0, help='Lamda')
parser.add_argument('--dataset', type=str, default='phy', help='Data set')
parser.add_argument('--percent', type=int, default=10, help='')
...
# non_zero_counts = non_zero_counts * 100
...
def train(epoch):
    ...
    output_list = []
    for k in range(K):
        output_list.append(torch.log_softmax(model(X, A), dim=-1))
    ...
    loss_train = loss_train + loss_kl + loss_consis
