...
from src.split_utils import random_planetoid_splits
...
parser.add_argument('--input_droprate', type=float, default=0.3,
                    help='Dropout rate of the input layer (1 - keep probability).')
parser.add_argument('--hidden_droprate', type=float, default=0.3,
                    help='Dropout rate of the hidden layer (1 - keep probability).')
parser.add_argument('--patience', type=int, default=100, help='Patience')
parser.add_argument('--K', type=int, default=5, help='Propagation step')
parser.add_argument('--sample', type=int, default=5, help='Sampling times of dropnode')
...
#idx_train, idx_val, idx_test = random_planetoid_splits(labels, 10, 500)

feature = torch.FloatTensor(features)

# 转换为NumPy数组
features_np = features.numpy()
labels_np = labels.numpy().reshape(-1, 1)  # 确保标签是二维的
...
# 合并降维后的特征和标签
combined = np.hstack((features_tsne, labels_np))

# 创建列名
columns = ['TSNE1', 'TSNE2', 'label']
...
sns.scatterplot(x='TSNE1', y='TSNE2', data=df, hue='label', palette='viridis')
plt.title('Original Pubmed')
plt.xlabel('')
plt.xticks([])#隐藏刻度
plt.ylabel('')
plt.yticks([])
...
def test():
    ...
    output_np = output_np.detach().numpy()
    ...
    # 使用t-SNE进行降维
    tsne = TSNE(n_components=2, random_state=42)
    features_tsne = tsne.fit_transform(output_np)
    ...
    # # 合并特征和标签
    # combined = np.hstack((output_np, labels_np))
    #
    # # 创建列名
    # output_np_columns = [f'feature_{i + 1}' for i in range(output_np.shape[1])]
    # columns = output_np_columns + ['label']
    #
    # # 将合并的数据转换为DataFrame
    # df = pd.DataFrame(combined, columns=columns)
    #
    ...
    #
    # plt.subplot(1, 2, 1)
    # sns.scatterplot(x='feature_1', y='feature_2', data=df, hue='label', palette='viridis')
